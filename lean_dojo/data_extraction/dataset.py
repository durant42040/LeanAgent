...
from ..constants import LEAN4_PACKAGES_DIR, __version__
from .lean import LeanGitRepo
from .trace import trace
from .traced_data import TracedRepo, TracedTheorem

# Import utility functions from the main project
import sys
...
def split_data(
    ...
    Args:
        ...
        num_val_pct: Percentage of theorems for validation (default: 0.02)
        num_test_pct: Percentage of theorems for testing (default: 0.02)
...
def export_data(
    ...
    **kwargs,
    ...
    logger.info("Successfully exported proofs")

    num_premises, num_files_traced = export_premises(traced_repo, dst_path)
    logger.info("Successfully exported premises")

    export_metadata(traced_repo, dst_path, **kwargs)
    logger.info("Successfully exported metadata")
...
def generate_dataset(url: str, commit: str, dst_dir: str) -> tuple:
    """
    ...
    """
    logger.info(f"Generating dataset: {url}@{commit} -> {dst_dir}")
